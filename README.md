...
The API this site used to consume is archived separately at
[rs-api-portfolio](https://github.com/meistyr/rs-api-portfolio).

## What the site does

- Nuxt 3 app with file-based routing and server-side rendering
- Nitro server routes under `server/api/` standing in for the original backend
- circuit browsing: teams, rosters, matchups, standings, and player stats across 22 divisions
- an operational portal view for team and roster summaries
- a hand-rolled SCSS design system (`assets/scss/`) with self-hosted OFL typefaces
- static content pages: rules, score reporting, shop, and socials

## Architecture

The app is unchanged in how it consumes data — only the source behind `server/api/` moved
from a live backend to a local snapshot.
...
No environment variables are required — see [`.env.example`](.env.example) for the optional
ones.
